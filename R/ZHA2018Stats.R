...
resultsTab = file.path('..', 'results', 'tables', 'ZHA2018_stats.csv')
...
for (removeGlobalOutliers in c(FALSE, TRUE))
  for (correctTrendDegree in c(-1,0,1))
    for (set in dataSets) {
      cat('\nData set : ', set, '\n')
...
# Save data
write.csv(dft2,file = resultsTab, row.names = FALSE)
...
df1 = data.frame(Dataset = d$Dataset, Methods = d$Methods)
for(s in stats) {
  v = d[[s]]
...
sink(file.path('..', 'results', 'tables', 'ZHA2018_Stats.tex'))
print(
  xtable::xtable(
    df1,
    type = 'latex',
